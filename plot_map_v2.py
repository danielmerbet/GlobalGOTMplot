...
base_cmap = plt.get_cmap('cmo.tempo', 256)  # vibrant and works great on dark backgrounds
extended_colors = base_cmap(np.linspace(0, 1, 512))  # Increase number of color steps
...
animated_plot = temp.hvplot.contourf(
    ...
    transform=ccrs.PlateCarree(),
    cmap=cmo.haline, #custom_cmap,  # Use the extended/custom colormap
    coastline=True,
    title='ISIMIP Global Simulations: Lake Surface Water Temperature',
    fontsize= 35,
